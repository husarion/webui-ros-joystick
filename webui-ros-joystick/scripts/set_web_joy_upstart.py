...
if (len(sys.argv) not in possible_arg_count) or len(sys.argv) == 1:
    ...
    Set up the autostart of webui ros joystick 
...
if str(sys.argv[1]) == "uninstall":
    ...
    subprocess.call("rm /etc/systemd/system/webui_ros_joy.service", shell=True)
...
subprocess.Popen(
    ['echo "{}" > /etc/systemd/system/webui_ros_joy.service'.format(rap_service)],
    shell=True,
)
